...
static void f_getchangelist(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
  ...
  if (!has_count) {
    ...
    if (count == 0) {
      list_T *const l = tv_list_alloc(1);
      tv_list_append_list(rettv->vval.v_list, l);
      dict_T *const d = tv_dict_alloc();
      tv_list_append_dict(l, d);
      tv_dict_add_nr(d, S_LEN("lnum"), buf->b_changelist[changelistindex].mark.lnum);
      tv_dict_add_nr(d, S_LEN("col"), buf->b_changelist[changelistindex].mark.col);
      tv_dict_add_nr(d, S_LEN("coladd"), buf->b_changelist[changelistindex].mark.coladd);
    } else if (count > 0) {
      int stop = changelistindex + count;
      stop = stop < buf->b_changelistlen ? stop : buf->b_changelistlen;
      list_T *const l = tv_list_alloc(stop - changelistindex);
      tv_list_append_list(rettv->vval.v_list, l);
      for (int i = changelistindex + 1; i <= stop; i++) {
        if (buf->b_changelist[i].mark.lnum == 0) {
          continue;
        }
        dict_T *const d = tv_dict_alloc();
      ...
      int stop = changelistindex + count;
      stop = stop > 0 ? stop : 0;
      list_T *const l = tv_list_alloc(changelistindex - stop);
      tv_list_append_list(rettv->vval.v_list, l);
      for (int i = changelistindex - 1; i >= stop; i--) {
        if (buf->b_changelist[i].mark.lnum == 0) {
          continue;
